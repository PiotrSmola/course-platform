  ...
  <div class="category-page">
    ...
    <div class="container category-body">
      ...
      <main class="category-main">
        <div v-if="isLoading" class="category-loading">
          <div class="spinner" />
          <p>Ładowanie kursów...</p>
        </div>

        <div v-else-if="isError" class="category-error">
          <p>Wystąpił błąd podczas ładowania kursów.</p>
          <button class="btn btn-primary" @click="resetFilters">Spróbuj ponownie</button>
        </div>

        <div v-else-if="filteredItems.length === 0" class="category-empty">
          <p>Brak kursów w tej kategorii spełniających wybrane kryteria.</p>
        ...
        <div v-if="totalPages > 1 && !isLoading" class="category-pagination">
...
const category = computed(() => allCategories.value.find((c) => c.slug === slug.value))
const categoryName = computed(() => category.value?.name ?? slug.value)
const categoryDescription = computed(
  () => category.value?.description ?? `Kursy z kategorii ${categoryName.value}. Znajdź materiały dopasowane do Twojego poziomu.`
)
...
watch(
  categoryId,
  (id) => {
    if (id && !state.categoryIds.includes(id)) {
